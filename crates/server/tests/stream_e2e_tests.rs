...
async fn start_test_server() -> TestServer {
    ...
    database.set_downloading_to_paused().await.expect("set paused");
    ...
    let app = streamx::server::build_router(
        database.clone(), config, torrent_engine, search_provider, hls_pipeline, log_tx, log_history,
    );
    ...
    let login_resp = client.post(format!("{base_url}/api/auth/login"))
        .json(&serde_json::json!({"username": "testuser", "password": "testpass123"}))
    ...
    TestServer { base_url, token, data_dir: tmp }
}
...
async fn playlist_requires_auth() {
    ...
    let resp = client.get(format!("{}/api/stream/fake_id/playlist.m3u8", server.base_url))
...
async fn playlist_returns_hls_for_h264() {
    ...
    let resp = client.get(format!(
        "{}/api/stream/{stream_id}/playlist.m3u8?quality=source", server.base_url
    )).send().await.expect("playlist request");

    assert_eq!(resp.status(), StatusCode::OK);
    let content_type = resp.headers().get("content-type").unwrap().to_str().unwrap();
    assert!(content_type.contains("mpegurl"), "Wrong content type: {content_type}");
    ...
    let resp2 = client.get(format!(
        "{}/api/stream/{stream_id}/playlist.m3u8?quality=source", server.base_url
    )).send().await.expect("playlist request 2");
    let body2 = resp2.text().await.expect("body2");
    assert!(body2.contains("#EXTINF:"), "No segments in playlist: {}", &body2[..200.min(body2.len())]);
}
...
async fn variant_segment_returns_valid_fmp4() {
    ...
    client.get(format!(
        "{}/api/stream/{stream_id}/playlist.m3u8?quality=source", server.base_url
    )).send().await.expect("trigger");

    tokio::time::sleep(std::time::Duration::from_secs(5)).await;

    // Fetch segment
    let resp = client.get(format!(
        "{}/api/stream/{stream_id}/source/segment_0000.m4s", server.base_url
    )).send().await.expect("segment request");

    assert_eq!(resp.status(), StatusCode::OK);
    let ct = resp.headers().get("content-type").unwrap().to_str().unwrap();
    assert!(ct.contains("mp4") || ct.contains("video") || ct.contains("octet-stream"), "Wrong content type: {ct}");

    let bytes = resp.bytes().await.expect("segment bytes");
    assert!(bytes.len() > 100, "Segment too small: {} bytes", bytes.len());
    let box_type = &bytes[4..8];
    assert!(
        box_type == b"styp" || box_type == b"moof" || box_type == b"mdat" || box_type == b"ftyp",
        "Invalid fMP4 box type: {:?}", box_type
    );
}
...
async fn quality_param_selects_tier() {
    ...
    // Request 360p
    client.get(format!(
        "{}/api/stream/{stream_id}/playlist.m3u8?quality=360p", server.base_url
    )).send().await.expect("360p trigger");

    tokio::time::sleep(std::time::Duration::from_secs(8)).await;

    // 360p directory should exist
    let resp = client.get(format!(
        "{}/api/stream/{stream_id}/360p/segment_0000.m4s", server.base_url
    )).send().await.expect("360p segment");
...
async fn stream_file_serves_complete_download() {
    ...
    let resp = client.get(format!(
        "{}/api/stream/{stream_id}/file", server.base_url
    )).send().await.expect("file request");

    assert_eq!(resp.status(), StatusCode::OK);
    let ct = resp.headers().get("content-type").unwrap().to_str().unwrap();
    assert!(ct.contains("video") || ct.contains("mp4"), "Wrong content type: {ct}");
...
async fn stream_file_supports_range_requests() {
    ...
    .send().await.expect("range request");

    assert_eq!(resp.status(), StatusCode::PARTIAL_CONTENT);
    let bytes = resp.bytes().await.expect("bytes");
    assert_eq!(bytes.len(), 1024, "Range response wrong size: {}", bytes.len());
}
...
async fn demo_playlist_returns_redirect() {
    ...
    let client = reqwest::Client::builder()
        ...
        .default_headers({
            ...
            h.insert("Authorization", format!("Bearer {}", server.token).parse().unwrap());
            h
        })
        .build().unwrap();

    let resp = client.get(format!(
        "{}/api/stream/demo/playlist.m3u8?quality=source", server.base_url
    )).send().await.expect("demo request");

    assert!(resp.status().is_redirection(), "Expected redirect, got: {}", resp.status());
    let location = resp.headers().get("location").unwrap().to_str().unwrap();
    assert!(location.contains("test-streams.mux.dev"), "Wrong redirect: {location}");
}
