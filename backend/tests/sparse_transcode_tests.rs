...
async fn transcode_to_segments(
    ...
    let mgr = HlsManager::new(&test_config(), cache_dir.clone()).await.unwrap();
...
async fn sequential_vs_sparse_same_segments(#[case] clip_id: &str) {
    let source_clip = match get_clip(clip_id) {
        Some(c) => c,
        None => { eprintln!("SKIP: clip not generated"); return; }
    };
    ...
    let (seq_seg, seq_names) = transcode_to_segments(&seq_path, "source", &format!("seq_{clip_id}")).await;

    assert!(!seq_seg.is_empty(), "Sequential: no segment produced");
    assert!(is_valid_fmp4(&seq_seg), "Sequential: invalid fMP4 segment");
    eprintln!("[{clip_id}] Sequential: {} segments, first={} bytes", seq_names.len(), seq_seg.len());
    ...
    let writer = SparseTorrentWriter::out_of_order(&source_clip, sparse_path.clone(), piece_size, 0)
        .unwrap();
    ...
    assert_eq!(sparse_data.len(), source_data.len(), "Sparse file size mismatch");
    assert_eq!(sparse_data, source_data, "Sparse file content mismatch - torrent simulation broken");

    // 3. Transcode the sparse-written file
    let (sparse_seg, sparse_names) = transcode_to_segments(&sparse_path, "source", &format!("sparse_{clip_id}")).await;

    assert!(!sparse_seg.is_empty(), "Sparse: no segment produced");
    assert!(is_valid_fmp4(&sparse_seg), "Sparse: invalid fMP4 segment");
    eprintln!("[{clip_id}] Sparse: {} segments, first={} bytes", sparse_names.len(), sparse_seg.len());

    // 4. Compare: same number of segments
    assert_eq!(seq_names.len(), sparse_names.len(),
        "Segment count differs: seq={} sparse={}", seq_names.len(), sparse_names.len());

    // 5. Compare: first segment identical
    assert_eq!(seq_seg.len(), sparse_seg.len(),
        "First segment size differs: seq={} sparse={}", seq_seg.len(), sparse_seg.len());
    assert_eq!(seq_seg, sparse_seg,
...
#[tokio::test]
async fn sparse_write_pattern_produces_valid_hls(
    #[case] pattern: &str,
    #[case] _delay_ms: u64,
) {
    ...
    let writer = match pattern {
        "sequential" => SparseTorrentWriter::sequential(&source_clip, output_path.clone(), piece_size, 10).unwrap(),
        "out_of_order" => SparseTorrentWriter::out_of_order(&source_clip, output_path.clone(), piece_size, 10).unwrap(),
        "slow_start" => SparseTorrentWriter::sequential_slow_start(&source_clip, output_path.clone(), piece_size).unwrap(),
        "stalling" => SparseTorrentWriter::stalling(&source_clip, output_path.clone(), piece_size, 500).unwrap(),
    ...
    assert_eq!(written, original, "[{pattern}] Written file differs from original");
    ...
    eprintln!("[{pattern}] PASS: {} segments, first={} bytes", names.len(), seg.len());
}
...
async fn sparse_file_quality_tiers(#[case] quality: &str) {
    ...
    // Write out of order
    let writer = SparseTorrentWriter::out_of_order(&source_clip, output_path.clone(), piece_size, 0).unwrap();
    writer.execute().await.unwrap();

    let (seg, names) = transcode_to_segments(&output_path, quality, &test_name).await;
    assert!(!seg.is_empty(), "[{quality}] No segments produced from sparse file");
    assert!(is_valid_fmp4(&seg), "[{quality}] Invalid fMP4 segment");
    eprintln!("[{quality}] PASS: {} segments from sparse HEVC file", names.len());
}
